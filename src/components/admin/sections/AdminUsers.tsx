...
import { 
  ...
  CheckCircle2, XCircle, ShieldCheck, UserCheck, UserX, AlertTriangle, X
} from 'lucide-react';
  ...
  const handleUserUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/users/${editingUser.id}`, {
        ...
        body: JSON.stringify({ role: editingUser.role, isActive: editingUser.isActive })
      });
        ...
        <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs">
          <div className="flex items-center justify-between pb-5 border-b border-slate-100 dark:border-slate-800 mb-6">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400 flex items-center justify-center border border-blue-200/60 dark:border-blue-800/40">
                <Shield className="w-5 h-5" />
              </div>
              <div>
                <h2 className="text-lg font-black text-slate-900 dark:text-white">
                  تعديل صلاحيات المستخدم: {editingUser.name || editingUser.email}
                </h2>
          ...
          <form onSubmit={handleUserUpdate} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                >
                  <option value="user">مستخدم عادي (المشاركة والتوقع فقط)</option>
                  <option value="admin">مسؤول معتمد (Admin - إدارة المحتوى والتوقعات)</option>
                </select>
                <p className="text-[11px] text-slate-400 mt-2 flex items-center gap-1.5">
                  <AlertTriangle className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                  <span>صلاحية المالك العام (Superadmin) محمية برمجياً لمنع التعديل غير المصرح به.</span>
                </p>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                >
                  <option value="true">نشط (مسموح له بتسجيل الدخول والتفاعل)</option>
                  <option value="false">معطل / محظور (ممنوع من الدخول للمنصة)</option>
                </select>
                <p className="text-[11px] text-slate-400 mt-2">
                  عند تعطيل الحساب، لن يتمكن المستخدم من الدخول أو إضافة توقعات جديدة.
                </p>
              </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
              <button
                ...
                className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2 text-sm shadow-xs disabled:opacity-60 active:scale-98"
              >
                {isActionLoading ? (
                  <>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden shadow-xs">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-200/80 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-4 pr-10 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs sm:text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none placeholder:text-slate-400"
            />
          </div>
          ...
          <select
            ...
            className="px-3.5 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-blue-500 outline-none"
          >
            <option value="">جميع الأدوار</option>
            <option value="superadmin">المالك العام (Superadmin)</option>
                ...
                <tr>
                  <td colSpan={5} className="py-16 text-center text-slate-400">
                    <div className="max-w-xs mx-auto space-y-2">
                      ...
                      <div className="text-xs text-slate-400">جرّب تعديل كلمة البحث أو فلتر الأدوار.</div>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 font-black text-xs flex items-center justify-center border border-slate-200 dark:border-slate-700">
                          ...
                          <div>
                            <div className="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-1.5">
                              ...
                              {u.email === 'abod46071@gmail.com' && (
                                <span className="bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 text-[10px] font-black px-2 py-0.5 rounded-full border border-purple-200 dark:border-purple-800">
                                  المالك الرئيسي
                                </span>
                              )}
                            </div>
                      ...
                      <td className="py-3.5 px-4">
                        {u.role === 'superadmin' ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-purple-50 text-purple-700 dark:bg-purple-950/50 dark:text-purple-300 border border-purple-200/60 dark:border-purple-800/40">
                            <Shield className="w-3 h-3 text-purple-600" />
                            <span>المالك العام</span>
                          </span>
                        ) : u.role === 'admin' ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-blue-50 text-blue-700 dark:bg-blue-950/50 dark:text-blue-300 border border-blue-200/60 dark:border-blue-800/40">
                      ...
                      <td className="py-3.5 px-4">
                        {u.isActive !== false ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-800/40">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                            <span>نشط</span>
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-rose-50 text-rose-700 dark:bg-rose-950/50 dark:text-rose-400 border border-rose-200/60 dark:border-rose-800/40">
                      ...
                      <td className="py-3.5 px-4 text-left">
                        {isProtected ? (
                          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800/80 px-2.5 py-1 rounded-lg">
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              ...
                              className="p-1.5 text-slate-600 hover:text-blue-600 hover:bg-blue-50 dark:text-slate-400 dark:hover:text-blue-400 dark:hover:bg-blue-950/40 rounded-lg transition-colors"
        ...
        <div className="grid grid-cols-1 gap-3 p-4 md:hidden">
          {isLoading ? (
            ...
            <div className="py-12 text-center text-xs text-slate-400">لم يتم العثور على أي مستخدم</div>
          ) : (
            filteredUsers.map((u) => {
              const isProtected = u.email === 'abod46071@gmail.com' || u.role === 'superadmin';
              return (
                <div
                  key={u.id}
                  className="bg-slate-50/70 dark:bg-slate-850 p-4 rounded-xl border border-slate-200/70 dark:border-slate-800 space-y-3"
                >
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="font-mono text-xs text-slate-400 mt-0.5" dir="ltr">
                        {u.email}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-200/60 dark:border-slate-800">
                    <span className="text-xs font-bold text-slate-500">
                      الدور: {u.role === 'superadmin' ? 'المالك العام' : u.role === 'admin' ? 'مسؤول (أدمن)' : 'مستخدم'}
                    </span>

                    {isProtected ? (
                      <span className="text-[11px] font-bold text-slate-400">محمي</span>
                    ) : (
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => setEditingUser(u)}
                          className="px-2.5 py-1 text-xs font-bold text-blue-600 bg-blue-50 dark:bg-blue-950/50 rounded-lg"
                        >
                          تعديل
                        </button>
                        <button
                          onClick={() => handleDeleteUser(u)}
                          className="px-2.5 py-1 text-xs font-bold text-rose-600 bg-rose-50 dark:bg-rose-950/50 rounded-lg"
                        >
                          حذف
                        </button>
                      </div>
